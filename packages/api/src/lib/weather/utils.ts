...
import type { WeatherData, RoadRisk, ApiProvider } from "./types";

// Weather code to precipitation type mapping (shared across providers)
export const PRECIPITATION_MAP: Record<number, WeatherData["precipitationType"]> = {
  // Tomorrow.io codes
  4000: "rain", // Drizzle
  4001: "rain", // Rain
...
export function getPrecipitationType(
  ...
  return PRECIPITATION_MAP[weatherCode] || (precipIntensity > 0 ? "rain" : "none");
}
...
export async function trackApiUsage(provider: ApiProvider, endpoint: string): Promise<void> {
