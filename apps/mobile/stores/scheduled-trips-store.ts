...
interface ScheduledTripsState {
  ...
  // Actions
  addTrip: (trip: Omit<ScheduledTrip, 'id' | 'createdAt' | 'isActive'>) => string;
  removeTrip: (tripId: string) => void;
  updateTripRecommendation: (tripId: string, recommendation: TripRecommendation) => void;
  ...
  persist(
    (set, get) => ({
      ...
      markNotificationSent: (tripId) => {
        set((state) => ({
          trips: state.trips.map((t) =>
            t.id === tripId ? { ...t, lastNotificationAt: new Date().toISOString() } : t
          ),
        }));
      },
      ...
      shouldNotify: (tripId) => {
        ...
        if (hoursUntilDeparture > trip.notifyHoursBefore || hoursUntilDeparture < 0) {
          return false;
        }
...
export function generateRecommendation(
  alerts: Array<{ severity: 'extreme' | 'severe' | 'moderate' | 'minor'; kmRange?: string }>,
  ...
  if (hasExtreme || severeCount >= 2) {
    return {
      ...
      details: 'Condiciones meteorológicas extremas en la ruta. Reprogramá tu viaje.',
    };
  }
  ...
  if (hasModerate) {
    return {
      ...
      details: 'Condiciones moderadas. Reducí la velocidad y mantené distancia.',
    };
  }
