...
export function useDrivingMode({
  ...
  const [manualMode, setManualModeState] = useState<DrivingModeState | null>(null);
  ...
  }, [speedKmh, mode, autoDetect, manualMode, enterThreshold, exitThreshold, sustainedDuration]);
...
export function useDrivingModeContext(): DrivingModeContextValue {
  ...
  if (!context) {
    throw new Error('useDrivingModeContext must be used within DrivingModeProvider');
  }
  return context;
}
