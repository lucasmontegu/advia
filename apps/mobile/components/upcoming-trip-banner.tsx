...
import {
	...
	useScheduledTripsStore,
} from "@/stores/scheduled-trips-store";
...
export function UpcomingTripBanner({
	onViewDetails,
	onDismiss,
}: UpcomingTripBannerProps) {
	const colors = useThemeColors();
	const nextTrip = useScheduledTripsStore((state) => state.getNextTrip());
	const deactivateTrip = useScheduledTripsStore(
		(state) => state.deactivateTrip,
	);
	const cleanupPastTrips = useScheduledTripsStore(
		(state) => state.cleanupPastTrips,
	);
	...
	return (
		...
		>
			...
			>
				...
				<View style={styles.statusContent}>
					...
					{recommendation.details && (
						<Text
							style={[styles.statusDetails, { color: colors.mutedForeground }]}
							numberOfLines={2}
						>
							{recommendation.details}
						</Text>
			...
			<View style={styles.actions}>
				...
				>
					<Text
						style={[styles.detailsButtonText, { color: colors.foreground }]}
					>
						Ver detalles
					</Text>
				</TouchableOpacity>
