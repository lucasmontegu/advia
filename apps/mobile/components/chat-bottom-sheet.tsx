...
export function ChatBottomSheet() {
	...
	const quickSuggestions = [
		{
			icon: "route" as const,
			text: t("map.suggestions.workRoute") || "¿Cómo está el clima en mi ruta?",
		},
		{
			icon: "alert" as const,
			text: t("map.suggestions.nearbyAlerts") || "¿Hay alertas cerca?",
		},
		{
			icon: "rain" as const,
			text: t("map.suggestions.willItRain") || "¿Va a llover hoy?",
		},
	];
	...
	const handleSend = useCallback(async () => {
		...
		try {
			...
			const errorMessage: ChatMessageData = {
				...
				role: "assistant",
				content:
					t("chat.error") || "Lo siento, hubo un error. Intenta de nuevo.",
				timestamp: new Date(),
			};
						...
						<View style={styles.headerActions}>
							...
							>
								<Icon
									...
									size={18}
									color={
										tts.isEnabled ? colors.primary : colors.mutedForeground
									}
								/>
							</Pressable>
							<Pressable
					...
					<View style={styles.collapsedContent}>
						<View style={styles.collapsedHeader}>
							...
							<View style={styles.collapsedTextContainer}>
								<Text
									style={[styles.collapsedTitle, { color: colors.foreground }]}
								>
